...
EXISTS veya NOT EXISTS ifadeleri de alt sorgudan getirilen degerlerin
içerisinde bir degerin
...
--MART ve NİSAN aylarında aynı urun_id ile satılan ürünlerin urun_id'lerini
  ve aynı zamanda bu ürünleri MART ayında alan musteri_isim 'lerini listeleyen 
  bir sorgu yazınız. 
*/
...
/*
varmış isimlendirip tek tek çağıracak
o sekilde yapınca aynı sütunları ikişer kere yazıyor
join yaptıgımız için
Bütün sorun sintex te on olduğu için oradaki aynı isimli
iki sütunun karıştırmamak için onlara isim vermesi lazım 
işim verincede tek tek yazması gerekir hepsini beraber yazaniyor

on  keywordu de hangi sütun üzerinden birleştirmen gerektiğini 
belirtiyor*/
...
-- SORU1: Tüm film_name'leri, tüm category'lerini ve filmlerde oynayan
