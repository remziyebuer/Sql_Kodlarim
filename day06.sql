...
select sehir from insan group by sehir; --ben yazdim bu kodu extra
...
select cinsiyet, round(avg(maas),1) as maas_ortalamasi from insan group by cinsiyet;
...
select ulke, sehir ,count (isim) from insan group by ulke, sehir;

-- SORU06: ulke'lere göre ve cinsiyet'e göre çalışan sayısını ve yas ortalamasını listeleyiniz.

select ulke , cinsiyet , count(isim) as calisan_sayisi, round(avg(yas)) as yas_ortalamasi from insan group by ulke , cinsiyet ;


-- SORU07: maas'ı 30000 den büyük olanların, ulke'lere göre ve cinsiyet'e göre çalışan 
--sayısını, yas ortalamasını listeleyiniz.

select ulke, cinsiyet, count(isim) as calisan_sayisi, round(avg(yas)) from insan 
...
3) Where komutundan farkı: Gruplama yaptıktan sonra cıkan sonucta şart kosulacaksa 
Having komutu kullanılır
*/

--SORU08: ulke'lere göre yas ortalamasını bulup, yas ortalaması 30'dan buyuk olanları 
--listeleyiniz. 

select ulke, round(avg(yas)) as yas_ortalamasi from insan group by ulke
having avg(yas)>30;

--SORU9: ulke'lere göre toplam maası bulup, toplam maas 30000'den buyuk olanları
--listeleyiniz
...
/* -----------------------------------------------------------------------------
  SORU3: sirket'i 'Honda', 'Ford' ve 'Tofas' olan personelin isim'ini tekrarsız
         listeleyin
------------------------------------------------------------------------------*/  
...
/* -----------------------------------------------------------------------------
  SORU5: isim'i 'Mehmet Ozturk' olanlarin isim'lerini, sehir'lerini ve
         sehir'i 'Istanbul' olmayanların isim'lerini sehir'lerini tekrarsız 
