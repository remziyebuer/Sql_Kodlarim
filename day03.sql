...
--SORU3: kursiyerler tablosundaki isim'i 'Derya Soylu' veya 'Yavuz Bal'
--olan satırları siliniz

delete from kursiyerler where isim = 'Derya Soylu' or 'Yavuz Bal';
...
--hocam sart olmadan  eltilerin savasini  recep ivedik yap diyemez miyiz?
update film set film_ismi = 'Recep İvedik' where film_ismi='Eltilerin Savasi';
...
--SORU3: arabalar tablosuna airbag varchar(10) seklinde yeni sutun 
--ekleyiniz default degeri 'mevcut' olsun.
...
1) UNIQUE -->  Bir sütundaki verilerin BENZERSİZ olmasıdır.
(2 veya daha fazla null deger kabul eder)

2) NOT NULL --> Bir sütunun NULL içermemesini sağlar
   (NOT NULL kısıtlaması veri türünden hemen sonra yazılır.
   CONSTRAINT ismi tanımlanmaz.)   

3) PRIMARY KEY --> UNIQUE(BENZERSİZ) - NOT NULL 
   Bir tabloda en fazla bir adet Primary Key olur. 
   iki sutunun birlesiminden bir Primary Key oluşturulmasına 
   ise Composite Primary Key denir.

4) CHECK --> Bir sütunun değer aralığını sınırlamak için kullanılır.

5) FOREIGN KEY --> Başka bir tablodaki PRİMARY KEY'i referans göstermek 
için kullanılır.
